  ...
  def newaccount
    if params[:commit] == "Cancel"
      ...
      if @newuser.save!
        ...
        # cookie our unique user id for retrieving this user after the session has expired
      ...
      if @authhash[:uid] != '' and @authhash[:provider] != ''
        ...
        if user_signed_in?
          ...
          if auth
            ...
            # if the user is signing back in they probably need the session recovery cookie
            cookies.permanent.signed[:user_id] = session[:user_id]
